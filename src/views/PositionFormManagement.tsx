...
import { Save, Plus, Trash2, ChevronDown, ChevronRight, ShieldAlert, GripVertical, Copy } from 'lucide-react';
...
export default function PositionFormManagement() {
  ...
  const updateCriterion = (idx: number, field: keyof PositionCriterion, val: string | number) => {
    if (!selectedConfig) return;
    const criteria = [...selectedConfig.criteria];
  ...
  const deleteCriterion = (idx: number) => {
    if (!selectedConfig) return;
    const criteria = selectedConfig.criteria.filter((_, i) => i !== idx);
    updateConfig(selectedConfig.id, { criteria });
  };
  ...
  const toggleCriterionStatus = (idx: number) => {
    if (!selectedConfig) return;
    const criteria = [...selectedConfig.criteria];
    criteria[idx] = { ...criteria[idx], status: criteria[idx].status === 'active' ? 'inactive' : 'active' };
    updateConfig(selectedConfig.id, { criteria });
  };
  ...
  const moveCriterionSection = (idx: number, sectionId: string) => {
    if (!selectedConfig) return;
    const criteria = [...selectedConfig.criteria];
    criteria[idx] = { ...criteria[idx], sectionId };
    updateConfig(selectedConfig.id, { criteria });
  };
                  ...
                  {sortedSections.map(section => {
                    ...
                    return (
                      <div key={section.id} className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                        <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-indigo-50 to-slate-50 dark:from-indigo-500/10 dark:to-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                        ...
                        {!isCollapsed && (
                          <div className="p-3 space-y-3 bg-white dark:bg-slate-800">
                            {sectionCriteria.length > 0 ? sectionCriteria.map((crit, ci) => {
                              ...
                              return (
                                <div key={crit.id} className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 relative group hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all">
                                  ...
                                  <div className="grid grid-cols-3 gap-3">
                                    ...
                                    <div>
                                      <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Move to Section</label>
